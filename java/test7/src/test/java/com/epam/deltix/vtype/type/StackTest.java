...
import com.epam.deltix.vtype.common.CrudeLogger;
...
public class StackTest {
    ...
    static JvmStack make(String str) {
        ...
        String[] items = Util.parseItemList(str);

        for (String item : items) {
            int typeId = Util.parseType(item);
    ...
    static boolean test(JvmStack vm, String str, boolean failExpected) {
        ...
        if (vm.top() != vm0.top()) {
            ...
            System.err.printf("Stack item count mismatch: Expected: %s%nActual : %s%n", Util.toStr(vm0), Util.toStr(vm));
            assertTrue(false);
        }
        ...
        for (int i = 0; i < vm.top(); i++) {
            if (vm.typeIdAt(i) != vm0.typeIdAt(i) || !vm.nameAt(i).equals(vm0.nameAt(i))) {
                ...
                System.err.printf("Stack mismatch at depth %d: Expected: %s%nActual : %s%n", i, Util.toStr(vm0, i), Util.toStr(vm, i));
                assertTrue(false);
            }
        }
        ...
        if (failExpected) {
            System.err.printf("Stack contents equality not expected: Expected: %s%nActual : %s%n", Util.toStr(vm0), Util.toStr(vm));
            assertTrue(false);
        }
    ...
    private void pushTypeId(int typeId) {
        vm.pushTypeId(typeId, Util.nameFor(typeId));
    }
    ...
    public void testPushPop() {
        ...
        pushTypeId(Util.t32id(2));
        expect( "1 2");

        pushTypeId(Util.t64id(3));
        expect( "1 2 3J");
        ...
        pushTypeId(Util.t64id(3));
        pushTypeId(Util.t32id(4));
        pushTypeId(Util.t32id(5));
        pushTypeId(Util.t64id(6));
        expect( "1 3J 4 5 6J");
