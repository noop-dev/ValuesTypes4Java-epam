...
import static com.epam.deltix.vtype.transformer.AsmUtil.*;
import static com.epam.deltix.vtype.transformer.Warnings.REF_ASSIGN;
import static com.epam.deltix.vtype.transformer.Warnings.UNBOXING_ASSIGN;
...
public class SinglePassCodeTransformer extends CodeTransformerBase {
    ...
    public void onAStore(VarInsnNode node) {
        ...
        if (TypeId.isVtNonArray(storedType)) {
            if (TypeId.vtValueDstSize(storedType) <= nSlots) {
                ...
                if (!TypeId.isVtValue(storedType)) {
    ...
    public void onAALoad(InsnNode node) {
        ...
        if (TypeId.isVt(arrayTypeId)) {
            if (TypeId.isVtArray(arrayTypeId)) {
                if (TypeId.isVtA1Ref(arrayTypeId)) {
                    ...
                    log.dbg("AALOAD X[]..[]");
                    vm.pop32();
                    vm.pushTypeId(TypeId.arrayFrom(TypeId.vtValueFrom(arrayTypeId), TypeId.getArrayDepth(arrayTypeId) - 1));
            ...
            vm.pop32();
            vm.pushTypeId(TypeId.getArrayElement(arrayTypeId));
        }
    }
    ...
    public void onDup64(InsnNode node) {
        ...
        if (!TypeId.isVtValue(stackType0) && !TypeId.isVtValue(stackType1)) {
            vm.dup2();
        } else {
    ...
    public void onDup32x(InsnNode node, int depth) {
        ...
        assert(TypeId.isVtValue(t0) || TypeId.isVtValue(t1));

        if (1 == depth) {
            if (TypeId.isVtValue(t0)) {
                // VT]
                if (TypeId.isVtValue(t1)) {
                    // VT VT]
    ...
    void transformLocalFrame(AbstractInsnNode node, FrameEntry frame, int stackOffset, boolean discardExtraVars) {
        ...
        if (needs64BitTypeInsertionOrDeletion) {
            ...
            for (int iSrc = 0, iDst = 0, nextSrc; iSrc < topSrc && iDst < nDst; iSrc = nextSrc, ++iDst) {
                ...
                if (TypeId.isSrc64(typeIdSrc) && TypeId.VOID == typeIdDst) {
