...
import com.epam.deltix.vtype.mapping.MethodDef;
import com.epam.deltix.vtype.type.*;
...
//                TypeId.toString(stackType, mapping), dstIndex, method.name, currentLine());
    ...
    protected AbstractInsnNode insertLoadVar(AbstractInsnNode node, int typeId, int aDst) {
        return insertBefore(node, new VarInsnNode(AsmUtil.loadOpcodeForTypeId(typeId), aDst));
    }

    protected AbstractInsnNode insertStoreVar(AbstractInsnNode node, int typeId, int aDst) {
        return insertBefore(node, new VarInsnNode(AsmUtil.storeOpcodeForTypeId(typeId), aDst));
    }
    ...
    protected void insertPop(AbstractInsnNode node, int typeId) {
        ...
        insertBasic(node, TypeId.isDst64(typeId) ? POP2 : POP);
    }
    ...
    protected AbstractInsnNode insertPop2nd(AbstractInsnNode node, int typeId0, int typeId1) {
        ...
        if (TypeId.isDst32(typeId0)) {
            if (TypeId.isDst32(typeId1)) {
    ...
    protected AbstractInsnNode insertArrayFill(AbstractInsnNode node, int typeId) {
        ...
        assert(TypeId.isVtA1Ref(vm.typeIdAt(0)));
    ...
    protected void popReferenceWithOptionalBoxing(AbstractInsnNode node, int cause) {
        ...
        if (TypeId.isVtValue(stackTypeId)) {
    ...
    protected void boxOrUnboxIfNeeded(int checkResult, int srcType, int dstType, AbstractInsnNode node, int cause) {
        ...
        switch (checkResult & ~TypeIdCast.HAS_VTYPE) {
    ...
    protected void transformMethodArgs(AbstractInsnNode node, int[] methodArgs, int nMethodArgs, int depthCorrected) {
        ...
        for (int i = 0; i < depthCorrected; ++i) {
            insertBefore(node, new VarInsnNode(AsmUtil.storeOpcodeForTypeId(vm.typeIdAt(i)), varTop + i * 2));
        }
        ...
        for (int i = depthCorrected - 1; i >= 0; --i) {
            ...
            insertBefore(node, new VarInsnNode(AsmUtil.loadOpcodeForTypeId(stackType), varTop + i * 2));
    ...
    protected void applyMethodArgs(AbstractInsnNode node, int[] methodArgs, int nMethodArgs, int checkResult) {
        ...
        if (nMethodArgs > 0) {
            ...
            if (TypeIdCast.SUCCESS != (checkResult & ~TypeIdCast.HAS_VTYPE)) {
    ...
    protected int transformMethodCall(MethodInsnNode node, MethodDef methodDef) {
        ...
        if (0 != (TypeIdCast.NO_SUBSTITUTION & checkResult))
            return TypeIdCast.NO_SUBSTITUTION;

        if (0 != (checkResult & TypeIdCast.NEED_SUBSTITUTION)) {
            assert(0 != (TypeIdCast.HAS_VTYPE & checkResult));
            ...
            if (0 != (checkResult & (TypeIdCast.NO_SUBSTITUTION | TypeIdCast.NEED_SUBSTITUTION))) {
                return TypeIdCast.NO_SUBSTITUTION;
            }
        }
...
//            }
    ...
    public void transformGetField(FieldInsnNode node) {
        ...
        if (TypeId.isVt(typeId)) {
            ...
            if (!TypeId.isVtValue(typeId) && !TypeId.isVtArray(typeId)) {
    ...
    public void transformPutField(FieldInsnNode node) {
        ...
        tryAddFieldInitInjectionPoint(node);
        // TODO: Rewrite
        if (TypeId.isVt(dstType)) {
            // Field names are not modified anymore
            //node.name = nameConverter.transform(node.name, desc);
            node.desc = mapping.typeIdToDstTypeDesc(dstType);
            if (TypeId.isVtValue(dstType)) {
                if (dstType == stackType) {
                    vm.popVType64();
                } else if (TypeId.isDst32(stackType)) {
            ...
            } else if (TypeId.isVtArray(dstType) && TypeId.isDst32(stackType)) {
            ...
            checkForOwnerTransformation(node); // ValueType fields will get new owner
        } else if (TypeId.isVtValue(stackType)) {
    ...
    public void onAReturn(InsnNode node) {
        ...
        tryAddFieldInitInjectionPoint(node);
        int typeId = vm.typeIdAt(0);
        vm.checkReturnStack(node, 1, 1, TypeId.isVtValue(typeId) ? 2 : 1);
        if (TypeId.isVt(returnTypeId)) {
            boxOrUnboxIfNeeded(node, typeId, returnTypeId, Warnings.REF_RETURN);
            if (TypeId.isVtValue(returnTypeId)) {
    ...
    public void onNew(TypeInsnNode node) {
        ...
        String desc = node.desc;
        if (TypeId.isVt(getClassTypeId(desc))) {
    ...
    public void onANewArray(TypeInsnNode node) {
        ...
        if (TypeId.isVt(typeId)) {
            ...
            } else if (TypeId.isVtArray(typeId)) {
                node.desc = mapping.typeIdToDstTypeDesc(typeId);
    ...
    public void onCheckCast(TypeInsnNode node) {
        ...
        assert(0 == (TypeIdCast.NEED_SUBSTITUTION & castResult));

        if (TypeId.isVtValue(stackTypeId)) {
            assert(0 == (TypeIdCast.NEED_BOXING & castResult)); // Impossible to require boxing in checkcast
            if ((TypeIdCast.SUCCESS | TypeIdCast.HAS_VTYPE) == castResult) {
            ...
            if (0 != (TypeIdCast.HAS_VTYPE & castResult)) {
                // Transform expected destination type
                if (TypeId.isVtArray(newTypeId)) {
                    if (0 == (TypeIdCast.NEED_UNBOXING & castResult) || TypeId.isNull(stackTypeId)) {
            ...
            if (TypeIdCast.SUCCESS == (castResult & ~TypeIdCast.HAS_VTYPE) || (TypeId.isVtNonArray(newTypeId) && TypeIdCast.NEED_UNBOXING == (castResult & ~TypeIdCast.HAS_VTYPE))) {
                // No further conversion required. Scalar Value Type is not unboxed
                return;
            }

            // Null check is correct?
            if (!TypeId.isNull(stackTypeId)) {
                // Boxing is done _before_ typecast, unboxing _after_ the typecast
                boxOrUnboxIfNeeded(castResult, stackTypeId, newTypeId, TypeIdCast.NEED_BOXING == castResult ? node : node.getNext(), Warnings.REF_CAST);
            }
    ...
    public void onInstanceOf(TypeInsnNode node) {
        ...
        if (TypeId.isVt(stackTypeId)) {
            if (TypeId.isVtValue(stackTypeId)) {
                ...
                if (TypeId.isVtArray(otherTypeId)) {
    ...
    protected void unboxForComparisonWith(AbstractInsnNode node, int thisId, int otherId) {
        ...
        if (TypeId.isVtRef(thisId)) {
            // Warning already present in the calling method
            insertUnboxing(node, thisId, thisId, -1);
        } else {
            assert(TypeId.isRefSrc(thisId));
            if (TypeId.isVtNonArray(otherId)) {
    ...
    public void onIfACmpEq(JumpInsnNode node, boolean isEquality) {
        ...
        boolean isVtScalar0 = TypeId.isVtNonArray(typeId0);
        boolean isVtScalar1 = TypeId.isVtNonArray(typeId1);
        ...
        assert(TypeId.isVtNonArray(typeId0) || TypeId.isVtNonArray(typeId1));
        assert(nVtScalar > 0);
        assert(!(2 == nVtScalar) || (TypeId.getVtClassIndex(typeId0) == TypeId.getVtClassIndex(typeId1)));

        if (TypeId.isVt(typeId0) && TypeId.isVt(typeId1)) {
            // Probably impossible, but..
            if (!TypeId.isSameVtClass(typeId0, typeId1) /*|| getArrayDepth(typeId0) != getArrayDepth(typeId1)*/) {
        ...
        } else if(TypeId.isNull(typeId0) || TypeId.isNull(typeId1)) {
            log.trace(isEquality ? "->IFNULL" : "->IFNONNULL");

            if (TypeId.isNull(typeId0)) {
        ...
        int nVtValue = TypeId.isVtValue(typeId0) ? 1 : 0;
        nVtValue += TypeId.isVtValue(typeId1) ? 1 : 0;
        ...
        if (TypeId.isRefDst(typeId1)) {
            assert(TypeId.isVtValue(typeId0));
            insertBasic(node, DUP2_X1);
            insertBasic(node, POP2);
            int tmp = typeId0;
            typeId0 = typeId1;
        ...
        if (TypeId.isVtRef(typeId0)) {
            log.dbg("Unbox 1 side of the comparison");
            insertUnboxing(node, typeId0, typeId0, -1);
            typeId0 = TypeId.vtValueFrom(typeId0); // Just in case
        }

        // Possible variants at this point:
        // VtValue ref
        // VtValue VtValue
        assert(TypeId.isVtValue(typeId1));
        assert(TypeId.isVtValue(typeId0) || !TypeId.isVtNonArray(typeId0));
        insertMethodStatic(node, classDef.getDstClassPath(), "isIdentical",
                TypeId.isVtValue(typeId0) ? "(JJ)Z" : "(JLjava/lang/Object;)Z");
    ...
    protected void popAny(AbstractInsnNode node, int n) {
        ...
        for (; n != 0; --n) {
            int typeId0 = vm.typeIdAt(0);
            if (n > 1 && !TypeId.isDst64(typeId0)&& !TypeId.isDst64(vm.typeIdAt(1))) {
                insertBasic(node, POP2);
                --n;
                vm.popMany(2);
            } else {
    ...
    public void onFrame(FrameNode node) {
        ...
        if (logDbg) {
            log.dbg("Stack Frame: %s", AsmUtil.stackFrameToString(node));
        }
    ...
    public void onPop64(InsnNode node) {
        ...
        if (TypeId.isVtValue(stackType0)) {
            vm.pop64();
            insertBasic(node, POP2);
            if (TypeId.isVtValue(stackType1)) {
    ...
    public void onSwap32(InsnNode node) {
        ...
        if (TypeId.isVtValue(stackType0)) {
            ...
            if (TypeId.isVtValue(stackType1)) {
                ...
                insertBefore(node, new VarInsnNode(AsmUtil.storeOpcodeForTypeId(stackType1), varTop + 2));
                insertBefore(node, new VarInsnNode(LLOAD, varTop));
                insertBefore(node, new VarInsnNode(AsmUtil.loadOpcodeForTypeId(stackType1), varTop + 2));
            }
            ...
            if (TypeId.isVtValue(stackType1)) {
                int varTop = varTop();
                insertBefore(node, new VarInsnNode(AsmUtil.storeOpcodeForTypeId(stackType0), varTop));
                insertBefore(node, new VarInsnNode(LSTORE, varTop + 2));
                insertBefore(node, new VarInsnNode(AsmUtil.loadOpcodeForTypeId(stackType0), varTop));
                insertBefore(node, new VarInsnNode(LLOAD, varTop + 2));
                remove(node);
            } else {
...
//    }
    ...
    public void onMethod(MethodInsnNode node) {
        ...
        if (0 != (checkResult & TypeIdCast.HAS_VTYPE) || TypeId.isVt(methodArgs[0])) {
            node.name = nameConverter.transformIf(node.getOpcode() == INVOKESTATIC || shouldBeRenamed(node.name, node.desc), node.name, node.desc);
            node.desc = DescriptorParser.getTransformedDesc(node.desc, false, mapping);
        }
        ...
        if (node.owner.charAt(0) == '[') {
            ...
            if (TypeId.isVt(ownerTypeId)) {
                if (!TypeId.isVtArray(ownerTypeId))
...
//    }
    ...
    public void onInvokeDynamic(InvokeDynamicInsnNode node) {
        ...
        if (0 != (checkResult & TypeIdCast.HAS_VTYPE) || TypeId.isVt(methodArgs[0])) {
            ...
            desc = node.desc = DescriptorParser.getTransformedDesc(node.desc, false, mapping);
            vtInDesc = true;
        }
        ...
        if (!vtInDesc && vtInInterface && vtInBody && vtInImplArgs && nInterfaceArgs == nImplArgs) do {
            ...
            if (TypeIdCast.SUCCESS != (checkResult2 & ~TypeIdCast.HAS_VTYPE))
                break;
            ...
            bsmArgs[0] = Type.getType(DescriptorParser.getTransformedDesc(interfaceTypeDesc, false, mapping));
            bsmArgs[1] = new Handle(bodyHandle.getTag(),
                    ...
                    DescriptorParser.getTransformedDesc(bodyTypeDesc, false, mapping),
                    bodyHandle.isInterface());

            bsmArgs[2] = Type.getType(DescriptorParser.getTransformedDesc(implTypeDesc, false, mapping));
