...
import static com.epam.deltix.vtype.transformer.AsmUtil.dumpVarNodes;
    ...
    public void remapIndex(LabelNode anchorLabel, int typeId, int aSrc, int aDst) {
        ...
        for (VarNodeWrapper v : active) {
            if (v.src == aSrc) {
                ...
                if (v.remapped && ((TypeId.isVt(typeId) && v.typeId != typeId) || v.varNode.index != aDst)) {
                    if (null == anchorLabel) {
                        ...
                        if (TypeId.isVtRef(typeId) && TypeId.isRefSrc(v.typeId) && v.varNode.index == aDst) {
