...
import com.epam.deltix.vtype.type.VariableMap;
...
public class AsmUtil {
    ...
    public static Object typeIdToFrameEntry(int typeId, String typeIdDesc) {
        ...
        if (TypeId.isVtRefOrVtArray(typeId))
            return typeIdDesc;
    ...
    private static int loadStoreOpcodeOffsetForTypeId(int typeId) {
        ...
        if (TypeId.isRefDst(typeId))     return ASTORE - ISTORE;
    ...
    public static boolean possibleTransformedVtSetter(int access, String name, String desc, Mapping mapping) {
        return isSetterFlags(access) && isSetterName(name)
                && DescriptorParser.isPossibleTransformedVtSetter(desc) && !DescriptorParser.findVtInMethodDesc(desc, mapping);
    }

    public static boolean appearsToBeVtSetter(String name, String desc, Mapping mapping) {

        return isSetterName(name)
                && DescriptorParser.findVtInMethodDesc(desc, mapping)
                //&& !newDesc.equals(desc)
                && DescriptorParser.isPossibleTransformedVtSetter(DescriptorParser.getTransformedDesc(desc, false, mapping));
    }
}