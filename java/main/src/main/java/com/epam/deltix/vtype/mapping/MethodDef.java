...
package com.epam.deltix.vtype.mapping;
...
public class MethodDef {
    ...
    public MethodDef(final ClassDef owner, String newOwner, String newName, String newDesc, String debugName) {
        ...
        try {
            numArgs = DescriptorParser.parseMethod(args, false, null, newDesc, owner.mapping);
        }
        catch (Exception e) {
    ...
    void initialize() {
        ...
        int ownerIndex = null != owner ? TypeId.getVtClassIndex(owner.typeId) : TypeId.VT_WILDCARD_INDEX;

        numArgs = DescriptorParser.parseMethod(args, isNonStaticSrc, null != mapping ? owner.getSrcClassPath() : null, oldDesc, mapping);
        if (numArgs == -1)
            throw new IllegalArgumentException(String.format("Unable to parse src: %s.%s", oldName, oldDesc));

        numArgs2 = DescriptorParser.parseMethod(args2, false, null, newDesc, mapping);
        if (numArgs2 == -1)
        ...
        if (args[0] != args2[0]) {
            if (!(TypeId.isVt(args[0]) /*&& ownerIndex == getVtClassIndex(args[0])*/ && newDesc.endsWith("J") && TypeId.getArrayDepth(args[0]) == TypeId.getArrayDepth(args2[0]))) {
        ...
        for (int i = 1; i <= numArgs; ++i) {
            if (args[i] != args2[i]) {
                if (!(TypeId.isVt(args[i]) /*&& ownerIndex == getVtClassIndex(args[i])*/ && TypeId.getArrayDepth(args[i]) == TypeId.getArrayDepth(args2[i]))) {
